...
def my_row_lookup(row: dict, *candidates: str, default: Any = None) -> Any:
    ...
    is_address = any("ADDRESS" in str(c).upper() for c in candidates)
    if is_address:
        for extra in ("Address", "ADDRESS", "City/Village", "Village / City", "Village/City", "City / Village", "Village", "village", "city/village", "village / city"):
            if extra not in enhanced_candidates:
                enhanced_candidates.append(extra)
...
def resolve_uid(row: dict) -> str:
    uid = my_row_lookup(row, "UID", "Id", "ID", "CONSULTATION ID", "Consultation ID", "ma ")
    if uid and not pd.isna(uid):
        ...
        if uid_str.lower() in ("date", "#ref!", "uid", "id", "unnamed"):
            return ""
        return uid_str
    return ""
...
def my_transform_prescription_items(row: dict, prescription_uuid: str, log: list) -> list[dict]:
    ...
    try:
        ...
        for start_index in start_indices:
            ...
            quantity_val = clean_text(_extract_block_value(keys, values, start_index, 1))
...
def _center_aliases(value: Any) -> list[str]:
    ...
    parts = [part.strip() for part in re.split(r"[,/\-\s]", lowered) if part.strip()]
    aliases = set()
    for part in parts:
...
def _resolve_center_row_for_sheet(center_rows: list[dict[str, Any]], sheet_ref: int | str | None) -> dict[str, Any] | None:
    if isinstance(sheet_ref, str):
        ...
        explicit_id_mappings = {
            "bhawadi": {"id": "e73c2c5f-6aea-4cfd-af7b-95acf4778f70", "name": "DigiSwasthya Telemedicine Centre, DS3 Bhawadi", "center_code": "DS3-Bhawadi"},
            "uk": {"id": "56137eeb-4720-4288-8ab2-1e2f71e096f3", "name": "DigiSwasthya Telemedicine Centre, DS6-UK", "center_code": "DS6-UK"},
            "ep": {"id": "28b98bcc-962f-47a9-9c36-a38f1a27b478", "name": "DigiSwasthya Telemedicine Centre, EP", "center_code": "EP"},
            "gujarat": {"id": "046f1796-84f8-4a3e-9b5a-d582079adca0", "name": "DigiSwasthya Telemedicine Centre, Gujarat", "center_code": "Gujarat"},
            "gujrat": {"id": "046f1796-84f8-4a3e-9b5a-d582079adca0", "name": "DigiSwasthya Telemedicine Centre, Gujarat", "center_code": "Gujarat"},
            "nalasopara": {"id": "2b57b31e-ddb6-4cb5-8b4a-1d7bdbee8955", "name": "DigiSwasthya Telemedicine Centre, Nalasopara", "center_code": "Nalasopara"},
            "sonbhadr": {"id": "b7d2b48d-2b46-4868-adb0-6dac1ef6c75a", "name": "DigiSwasthya Telemedicine Centre, Sonbhadra", "center_code": "Sonbhadra"},
            "sonbhadra": {"id": "b7d2b48d-2b46-4868-adb0-6dac1ef6c75a", "name": "DigiSwasthya Telemedicine Centre, Sonbhadra", "center_code": "Sonbhadra"},
        }
        for key, info in explicit_id_mappings.items():
            if key in ref_lower:
                for row in (center_rows or []):
                    if str(row.get("id", "")).strip().lower() == info["id"].lower():
                        return row
                return info
        ...
        explicit_mappings = {
            ...
            "asharafpur": "DS-TMC-003"
        }
        for key, target_code in explicit_mappings.items():
            if key in ref_lower:
                for row in (center_rows or []):
        ...
        for row in center_rows:
            ...
            for alias in row_aliases:
                if alias == normalized or alias in sheet_aliases or normalized in alias or alias in normalized:
                    return row
            for alias in row_aliases:
...
def clean_doctor_name(val: Any) -> str:
    ...
    corrections = {
        ...
        "bhuvneshchaturvedicamp": "bhuvneshchaturvedi",
...
# =============================================================================
# NATIVE-DATA PROTECTION GUARD
# The Excel load runs in UPDATE mode. It must NEVER modify native rows
# (legacy_source IS NULL) or any row outside the migration set. It may only
# update rows whose legacy_source is one of the migration-origin values below.
# Comparison is case-insensitive. See excel-migration-legacy-source-guard memo.
# =============================================================================
_EXCEL_UPDATE_ELIGIBLE_SOURCES = {
    "dgiswasthya_database",    # legacy-DB patient rows
    "digiswasthya_database",   # legacy-DB visit/prescription rows
    "excel_digiswasthya",      # rows created by prior Excel runs
    "excel+database",          # rows already updated by a prior Excel run
}


def _is_native_or_ineligible_source(legacy_source: Any) -> bool:
    """True if a row must NOT be touched (native NULL, or any non-migration source)."""
    if legacy_source is None:
        return True
    return str(legacy_source).strip().lower() not in _EXCEL_UPDATE_ELIGIBLE_SOURCES


def _record_is_update_eligible(conn, schema: str, table: str, record_id: str) -> bool:
    """
    True only if the existing DB row's legacy_source is in the migration set.
    Native rows (legacy_source IS NULL) and any unknown source return False,
    so they are never updated and never linked to as a migration target.
    """
    try:
        row = conn.execute(
            text(f"SELECT legacy_source FROM {_table(schema, table)} WHERE id = :id LIMIT 1"),
            {"id": record_id},
        ).mappings().first()
    except Exception:
        return False
    if not row:
        return False
    return not _is_native_or_ineligible_source(row.get("legacy_source"))
...
def _has_actual_updates(conn, schema: str, table: str, record_id: str, transformed_row: dict[str, Any]) -> bool:
    ...
    try:
        ...
        # GUARD: never report updates for native/ineligible rows — they must not be touched.
        if table in ("patient", "visit", "prescription") and _is_native_or_ineligible_source(db_row.get("legacy_source")):
            return False

        for col, excel_val in transformed_row.items():
            if col in ("id", "created_at", "migrated_at", "legacy_source", "tenant_id", "is_demo"):
                continue
            if col == "legacy_id":
                if db_row.get("legacy_id") is None or str(db_row.get("legacy_id")).strip() == "":
                    if excel_val and str(excel_val).strip() != "":
                        return True
                continue
...
def _update_record_fields(conn, schema: str, table: str, record_id: str, transformed_row: dict[str, Any], uid: str, log: list, force_legacy_source: bool = False) -> bool:
    ...
    try:
        ...
        # GUARD: never update native/ineligible rows (legacy_source IS NULL or outside the
        # migration set). This protects all live/native data across patient/visit/prescription.
        if table in ("patient", "visit", "prescription") and _is_native_or_ineligible_source(db_row.get("legacy_source")):
            return False
        ...
        for col, excel_val in transformed_row.items():
            ...
            if col == "legacy_id":
                if db_row.get("legacy_id") is None or str(db_row.get("legacy_id")).strip() == "":
                    if excel_val and str(excel_val).strip() != "":
                        update_fields["legacy_id"] = str(excel_val).strip()
...
def main():
    ...
    try:
        with engine.connect() as conn:
            ...
            # Centers
            centers_to_ensure = [
                {"id": "e73c2c5f-6aea-4cfd-af7b-95acf4778f70", "name": "DigiSwasthya Telemedicine Centre, DS3 Bhawadi", "center_code": "DS3-Bhawadi", "is_active": True},
                {"id": "56137eeb-4720-4288-8ab2-1e2f71e096f3", "name": "DigiSwasthya Telemedicine Centre, DS6-UK", "center_code": "DS6-UK", "is_active": True},
                {"id": "28b98bcc-962f-47a9-9c36-a38f1a27b478", "name": "DigiSwasthya Telemedicine Centre, EP", "center_code": "EP", "is_active": True},
                {"id": "046f1796-84f8-4a3e-9b5a-d582079adca0", "name": "DigiSwasthya Telemedicine Centre, Gujarat", "center_code": "Gujarat", "is_active": True},
                {"id": "2b57b31e-ddb6-4cb5-8b4a-1d7bdbee8955", "name": "DigiSwasthya Telemedicine Centre, Nalasopara", "center_code": "Nalasopara", "is_active": True},
                {"id": "b7d2b48d-2b46-4868-adb0-6dac1ef6c75a", "name": "DigiSwasthya Telemedicine Centre, Sonbhadra", "center_code": "Sonbhadra", "is_active": True},
            ]
            for c_info in centers_to_ensure:
                conn.execute(text(f"INSERT INTO {_table(cfg.schema, 'center')} (id, name, center_code, is_active) VALUES (:id, :name, :center_code, :is_active) ON CONFLICT (id) DO NOTHING"), c_info)
            conn.commit()
    ...
    for sheet_name in sheets_to_process:
        ...
        if not resolved_center:
            print(f"      [SKIP] Sheet '{sheet_name}' does not map to any database center. Skipping this sheet.")
            continue
        resolved_center_id = str(resolved_center.get("id"))
        ...
        for idx, (_, row) in enumerate(sheet_df.iterrows(), start=1):
            ...
            if not doctor_uuid:
                doctor_uuid = FALLBACK_DOCTOR_USER_ID
    ...
    # TEST-RUN LIMIT: process only the first N aggregated records when MIGRATION_ROW_LIMIT is set.
    _row_limit = os.getenv("MIGRATION_ROW_LIMIT")
    if _row_limit:
        try:
            _n = int(_row_limit)
            if 0 < _n < len(excel_row_data):
                print(f"[LIMIT] MIGRATION_ROW_LIMIT={_n} -> processing only first {_n} of {len(excel_row_data)} records (TEST RUN)")
                excel_row_data = excel_row_data[:_n]
                stats["total_migratable_records"] = len(excel_row_data)
        except ValueError:
            print(f"[LIMIT] Ignoring invalid MIGRATION_ROW_LIMIT={_row_limit!r}")
    ...
    if os.getenv("PATIENTS_ONLY") == "1":
        print("PATIENTS_ONLY=1 -> stopping after Phase A (patients). Phase B skipped.")
        choice = "n"
    ...
    try:
        with engine.connect() as conn:   # managed manually so we can commit in batches (resilient to drops)
            # --- LOOP 1: VISITS ---
            print("\nMigrating 'visit' table...")
            for idx, item in enumerate(excel_row_data, start=1):
                raw = item["raw"]
                ...
                patient_row = item["patient_row"]
                pat_ids_all = _find_matching_patient_ids_with_age_fallback(conn, cfg.schema, patient_row)
                # GUARD: never let the age-fallback attach this visit to a native/ineligible patient.
                pat_ids = [
                    pid for pid in pat_ids_all
                    if _record_is_update_eligible(conn, cfg.schema, "patient", pid)
                ]
                if pat_ids:
                    supabase_patient_id = pat_ids[0]
                else:
                    # Fall back to the patient mapped/inserted in Phase A (guaranteed eligible)
                    supabase_patient_id = patient_id_map[orig_pat_id]
                ...
                if idx % 100 == 0 or idx == len(excel_row_data):
                    conn.commit()   # batch commit — durable progress, survives a later connection drop
            ...
            print("\nMigrating 'prescriptionitem' table...")
            for idx, item in enumerate(excel_row_data, start=1):
                raw = item["raw"]
            ...
            print("\nMigrating 'followup_schedule' table...")
            for idx, item in enumerate(excel_row_data, start=1):
                raw = item["raw"]
                orig_pat_id = item["orig_patient_id"]
